...
DEFAULT_START_ID = 30001  # ĐÃ SỬA MỐC KHỞI ĐIỂM THÀNH 30001
...
# Hàng chờ lưu 60 dòng log mới nhất
...
def get_progress_from_notion() -> int:
    ...
    return DEFAULT_START_ID - 1  # Trả về 30000 nếu chưa có lịch sử
...
def fetch_all_from_notion() -> list:
    url = f"https://api.notion.com/v1/databases/{NOTION_DATABASE_ID}/query"
    results = []
    seen_ids = set()  # BỘ LỌC KHỬ TRÙNG DỮ LIỆU HIỂN THỊ
    has_more = True
    next_cursor = None
    ...
    while has_more:
        ...
        try:
            ...
            for page in data.get("results", []):
                ...
                # Nếu ID đã xuất hiện trong danh sách rồi thì bỏ qua
...
# === SỬA LỖI DOUBLE: CHỈ CHO PHÉP 1 TIẾN TRÌNH CHÍNH CHẠY AUTO-START ===
def auto_start_scanner():
    time.sleep(3)
    # Ngăn Flask Debug Reloader nhân đôi tiến trình khi chạy local
