...
# Header giả lập trình duyệt Chrome thật 100% chống Bilibili ngắt kết nối
DEFAULT_HEADERS = {
...
def run_background_scanner(start_id, end_id, threads, delay_ms):
    ...
    def scan_task(media_id):
        ...
        if not is_in_notion(media_id):
            ...
            # Cơ chế tự động thử lại (Auto Retry 3 lần) chống rớt mạng / ngắt kết nối 10054
            max_retries = 3
            for attempt in range(max_retries):
                if not scanner_state["is_running"]:
                    break
                try:
                    # Mỗi request dùng một Session riêng biệt để tránh đụng độ Socket đa luồng
                    ...
                    break  # Request thành công -> Thoát vòng lặp thử lại
                except Exception as e:
                    if attempt < max_retries - 1:
                        print(f"  [RETRY {attempt+1}/{max_retries}] Lỗi kết nối ID {media_id}: Đang thử lại sau 1.5s...")
                        time.sleep(1.5)
                    else:
                        print(f"  [FAILED] Không kết nối được ID {media_id} sau {max_retries} lần thử: {e}")
