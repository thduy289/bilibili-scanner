    ...
    if not is_in_notion(media_id):
        ...
        for attempt in range(max_retries):
            if not scanner_state["is_running"]:
                break
            try:
                # Dùng context manager `with` để tự động đóng Session giải phóng RAM
...
def run_background_scanner(start_id, end_id, threads, delay_ms):
    ...
    # Hàng chờ giới hạn tối đa 100 ID trong RAM (chống tràn 512MB RAM Render)
    ...
    # Nạp từng ID vào hàng chờ từ từ
