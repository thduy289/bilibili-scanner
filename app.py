...
# === BỘ NHỚ RAM CACHE GIÚP WEB TẢI TỨC THÌ (0.001s) ===
results_cache = []
...
def is_in_notion(media_id: int) -> bool:
    # Kiểm tra nhanh trong bộ nhớ RAM trước
    with results_lock:
...
def save_to_notion(item: dict):
    # Lập tức đẩy phim mới vào RAM Cache để giao diện Web hiển thị ngay
    with results_lock:
...
def load_initial_cache_from_notion():
    """Tải toàn bộ dữ liệu Notion vào RAM ngay khi ứng dụng khởi động"""
    global results_cache
    add_log("🔄 Đang tải dữ liệu từ Notion vào bộ nhớ RAM...")
    ...
    add_log(f"⚡ Tải xong {len(results)} phim từ Notion vào RAM!")
...
def run_background_scanner(threads=2, delay_ms=500):
    ...
    # Bật trạng thái đang quét ngay từ giây đầu tiên
    with state_lock:
        scanner_state["is_running"] = True

    # Nạp dữ liệu ban đầu vào RAM
    load_initial_cache_from_notion()

    start_id = get_progress_from_notion() + 1
    add_log(f"🚀 Bắt đầu quét từ ID: {start_id}")
...
# Trả về kết quả từ RAM trong 0.001 giây (Không bắt Web chờ Notion)
