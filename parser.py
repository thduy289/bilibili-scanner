...
def parse_media_info(html_content: str, media_id: int) -> dict | None:
    ...
    try:
        # 1. Bỏ qua nếu trang báo lỗi hoặc video bị gỡ
        if soup.select_one(".bstar-error"):
            print(f"  [ID {media_id}] ❌ Trang không tồn tại (.bstar-error)")
            return None
        ...
        # Tìm bằng regex dự phòng nếu class thay đổi
        if not region:
        ...
        # 3. Trích xuất Title (Tiêu đề tiếng Anh)
