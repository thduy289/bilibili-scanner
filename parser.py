...
def parse_media_info(html_content: str, media_id: int, log_func=None) -> dict | None:
    ...
    try:
        # 1. Bỏ qua nếu trang lỗi hoặc bị gỡ
        ...
        if log_func: log_func(f"🎉 ID {media_id}: TÌM THẤY PHIM JAPAN '{title}'")
        ...
        # Tiêu hủy cây DOM của BeautifulSoup để giải phóng RAM lập tức
        soup.decompose()
