...
def parse_media_info(html_content: str, media_id: int) -> dict | None:
    ...
    # 1. Bỏ qua nếu là trang lỗi
    if soup.select_one(".bstar-error"):
        print(f"  [ID {media_id}] ❌ Trang báo lỗi (Trang không tồn tại)")
        return None

    if "This video has been removed" in html_content or "Video này đã bị gỡ" in html_content:
        print(f"  [ID {media_id}] ❌ Video này đã bị gỡ")
        return None
    ...
    # Nếu không tìm thấy qua class, tìm bằng regex trong HTML
    if not region:
    ...
    if not region:
        print(f"  [ID {media_id}] ⚠️ Không tìm thấy thông tin Region")
        return None

    print(f"  [ID {media_id}] 📍 Region tìm được: '{region}'")

    if "japan" not in region.lower():
        print(f"  [ID {media_id}] ⏭️ Bỏ qua vì Region là '{region}' (Không phải Japan)")
        return None
